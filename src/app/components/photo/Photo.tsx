...
export interface PhotoProps {
  /** Eyebrow label, e.g. "Most replayed this month" */
  title?: string;
  /** Exactly 4 photos render best; the fan/grid math assumes 4 */
  photos: PhotoItem[];
  instagramUrl?: string;
}

export default function Photo({ photos, instagramUrl }: PhotoProps) {
  const bem = new BemBuilder("photo", styles);
  return (
    <section className={bem.block()}>
      <div className={bem.element("grid")}>
        {photos.map(({ src, alt }: PhotoItem, index) => (
          <div key={index} className={bem.element("entry")}>
